...
if __name__ == '__main__':
    ...
    if False:
        ...
        sum_xyz_sigmas = None
        test_frame_number = min(len(input_out_file_folder), 200)
    ...
    index_bad_point = np.argmin((distance / visible_counter) * (1/(sigmas / visible_counter) + np.where(visible_counter >= np.max(visible_counter) * 0.9, 1, np.max(distance) * 1000)))
    ...
    for _ in range(number_of_processes):
        worker = Process(target=read_file_point_loop, args=(file_path_queue, shared_mem, point_id_for_cycle))
        worker.start()
        workers.append(worker)
    ...
    #coordinates = coordinates[0:350] # TODO: Zum Verwednen der ersten X Frames zum Ausrichten des Koordinatensystems
    ...
    for aoi_id in range(len(available_aoi_ids)):
        index_highest_AOI_on_blade = np.where((blade_number_of_aoi == blade_number_of_aoi[aoi_id]) & (aoi_to_blade_aoi == np.max(aoi_to_blade_aoi)))[0][0]
        rotation_matrix_for_aoi[aoi_id] = find_x_rotation_matrix(mean_position_array[index_highest_AOI_on_blade])
    ...
    for aoi_index in range(len(available_aoi_ids)):
            ...
            for j in range(i):
                ...
                for k in range(0,10):
                    if value < current_best_value_list[k]:
                        ...
                        best_points_list.insert(k, np.array([indices_for_aoi_local[i], indices_for_aoi_local[j]]))
        ...
        for i in range(len(best_points_list)):
            assert best_points_list[i] is not None , "Zu wenig Punkte um 25 gute Punktepaare zu finden"
            if coordinates[best_points_list[i][0]][1] > coordinates[best_points_list[i][1]][1]:
                best_points_list[i][0], best_points_list[i][1] = best_points_list[i][1], best_points_list[i][0]
    ...
    for aoi_index in range(len(available_aoi_ids)):
        for i in range(0, len(index_array[aoi_index]), 2):
            ...
            ax.plot([coordinates[index_start][0], coordinates[index_end][0]], [coordinates[index_start][1], coordinates[index_end][1]],zs=[coordinates[index_start][2], coordinates[index_end][2]])
            ax.scatter([coordinates[index_start][0], coordinates[index_end][0]], [coordinates[index_start][1], coordinates[index_end][1]],zs=[coordinates[index_start][2], coordinates[index_end][2]], s = 80)  


    ax.scatter(coordinates[indices_found].T[0], coordinates[indices_found].T[1], coordinates[indices_found].T[2], c = 'b', alpha=.35, s = 5)
    ...
    shared_mem = SharedMemory([np.empty((len(interesting_subsets_id_vicpy), 51, len(variables_export_name_out_file)), np.float32)])
    file_path_queue = Queue(maxsize=30)
    ...
    good_points_torsion = np.empty((len(input_out_file_folder), len(available_aoi_ids), 50, len(variables_export_name_out_file) ), float)
    ...
    for i in range(0,50,2):
        for aoi_id in range(len(available_aoi_ids)):
            ...
            for var_id in range(1, len(variables_export_name_csv_file)):
                ...
                dict1[variables_export_name_csv_file[var_id]] = good_points_torsion[:,aoi_id, i, var_id-1]
                dict2[variables_export_name_csv_file[var_id]] = good_points_torsion[:,aoi_id, i+1, var_id-1]
                ...
                csv_filename2 = output_path4 + "/Tor_" + str(i//2) + "/Blade_" + str(int(blade_number_of_aoi[aoi_id])) + "_AOI_" + str(int(aoi_to_blade_aoi[aoi_id])) +  "_P1.csv"
                csv_filename3 = output_path4 + "/Tor_" + str(i//2) + "/Blade_" + str(int(blade_number_of_aoi[aoi_id])) + "_AOI_" + str(int(aoi_to_blade_aoi[aoi_id])) +  "_P2.csv"
